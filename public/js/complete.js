...
function setPaymentDetails(intent) {
  ...
  switch (intent.currency) {
    ...
    case "eur":
      // cents → dollars/euros
      formattedAmount = (intent.amount / 100).toFixed(2);
    case "jpy":
      // yen はそのまま
      formattedamount = intent.amount.toString();
    default:
      formattedamount = intent.amount.toString();
  }
  ...
  switch (intent.currency) {
    ...
    case "eur":
      ...
      break;
    default:
      formattedCurrency = intent.currency;
      break;
  }
  ...
  document.querySelector("#intent-amount").textContent = `${formattedCurrency}${formattedAmount}`;
